...
void setup() {
    ...
    // Data freshness dot — at ~340° (upper-left, 20° before 12 o'clock) on the round face
    // 340° clock: x = 240 + 210*sin(340°) ≈ 168, y = 240 - 210*cos(340°) ≈ 43; top-left of 18px dot = (159, 34)
    {
        lv_obj_t* layer = lv_layer_top();
        s_data_dot = lv_obj_create(layer);
        lv_obj_set_size(s_data_dot, 18, 18);
        lv_obj_set_pos(s_data_dot, 159, 34);   // 340° position on 480x480 circular face
...
void loop() {
    ...
    // Data freshness indicator: update dot color + blink continuously
    if (s_data_dot) {
        ...
        // Update colour when state changes
        DotState state;
        if (last_upd == 0) {
            state = DS_NEVER;
        } else {
