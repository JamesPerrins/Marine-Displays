...
void gauge_number_display_update_center(int screen_num, float value, const char* unit, const char* description, const char* display_name) {
    ...
    const char* safe_display_name = (display_name && display_name[0] != '\0') ? display_name : "";
    if (strcmp(description,        prev_gauge_num_center_description[screen_num])  != 0 ||
        strcmp(safe_display_name,  prev_gauge_num_center_display_name[screen_num]) != 0) {
        // Prefer Signal K meta.displayName; fall back to acronym from description
        String label = (safe_display_name[0] != '\0') ? String(safe_display_name)
        ...
        strncpy(prev_gauge_num_center_display_name[screen_num], safe_display_name, sizeof(prev_gauge_num_center_display_name[screen_num]) - 1);
    }
}
