...
static String extract_path_scope(const String& prefix) {
    ...
    if (second_slash < 0) return "";
    int third_slash  = prefix.indexOf('/', second_slash + 1);
    if (third_slash < 0) return "";
    return prefix.substring(third_slash + 1);
}
...
static void mqtt_callback(char* topic, byte* payload, unsigned int length) {
    ...
    // Diagnostic: log first 5 messages so the user can verify topic format
...
static bool mqtt_connect() {
    ...
    if (s_topic_prefix.length() > 0) {
        ...
        s_prefix_len = s_topic_prefix.length() + 1;
    } else {
        sub_topic    = "#";
        s_prefix_len = 0;
    }
    ...
    while (s_mqtt_enabled) {
        ...
        if (!s_mqtt_client.connected()) {
            ...
            if (s_keepalive_topic.length() > 0 &&
                ...
                last_keepalive = now;
                bool ka_ok = s_mqtt_client.publish(s_keepalive_topic.c_str(),
                                                   s_keepalive_payload.c_str());
                Serial.printf("[MQTT] Keepalive → %s (%s)\n",
                              s_keepalive_topic.c_str(), ka_ok ? "OK" : "FAILED");
            }
        }
