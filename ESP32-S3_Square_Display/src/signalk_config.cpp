...
// Uses the /meta sub-endpoint (flat JSON) to avoid buffer overflow from the
// full path response; also surfaces displayName alongside units/description.
...
void fetch_all_metadata() {
    ...
    for (const String& path : all_paths) {
        ...
        if (!in_gauge) {
            // Fetch metadata via /meta sub-endpoint (flat JSON: units/description/displayName)
